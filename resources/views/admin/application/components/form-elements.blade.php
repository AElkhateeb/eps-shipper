...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('job_id'), 'has-success': fields.job_id && fields.job_id.valid }">
    <label for="job_id" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.application.columns.job_id') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.job_id" v-validate="'required|integer'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('job_id'), 'form-control-success': fields.job_id && fields.job_id.valid}" id="job_id" name="job_id" placeholder="{{ trans('admin.application.columns.job_id') }}">
        <div v-if="errors.has('job_id')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('job_id') }}</div>
    </div>
</div>
